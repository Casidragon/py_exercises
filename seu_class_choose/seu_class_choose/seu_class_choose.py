...
class LoginDialog(Toplevel):
    def __init__(self, parent, title=None):
        ...
        self.btn_submit = Button(self, text='   提交   ',command = lambda: self.destroy()) #command=lambda: self.destroy())
    ...
    def submit_login(self):
        ...
        id = threading.Thread(target = self.init_data) #_thread.start_new_thread(self.init_data, (3,))
        id.setDaemon(True)
        id.start()
        pass
    ...
    def doPost(self, step):
        ...
        if step == 1:
            ...
            values = {
                ...
                'userId': username}
            data = urllib.parse.urlencode(values).encode('utf-8')           #urlencode(values)
            headerNum = random.randint(0, 3)
        ...
        if step == 2:
            ...
            req2 = urllib.request.Request(url2, headers=header2)
        ...
        if step == 3:
            ...
            req3 = urllib.request.Request(url3, headers=header3)
        ...
        if step == 4:
            ...
            req4 = urllib.request.Request(url4, headers=header4)
        ...
        if step == 5:
            ...
            req5 = urllib.request.Request(url5, headers=header5)
        ...
        if step == 6:
            ...
            req6 = urllib.request.Request(url6, headers=header6)
...
def select_worker(typo, cid, index):
    ...
    while cid in list_selecting:
        ...
        if typo == 0:
            ...
            req6 = urllib.request.Request(url, headers=header)
            try:
                response = urllib.request.urlopen(req6, timeout=3)
            except Exception as e:
        ...
        if typo == 1:
            ...
            req6 = urllib.request.Request(url, headers=header)
            try:
                response = urllib.request.urlopen(req6, timeout=5)
            except Exception as e:
...
def get_verifycode():
    ...
    img = urllib.request.urlopen('http://xk.urp.seu.edu.cn/jw_css/getCheckCode', timeout=8)
